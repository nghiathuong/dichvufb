<?php 
    ...
    if (!isset($_SESSION['email_user'])) {
        ...
        if(empty(check_string($_POST['link_post'])) || empty(check_string($_POST['server'])) || empty(check_string($_POST['camxuc'])) || empty(check_string($_POST['soluong']))){
            ...
            //
            $check_suite = $DUONGSHADO->get_row("SELECT * FROM service_server WHERE code_server = 'like_post_sale' AND status_server = 'active' AND key_server = 'facebook_service' AND server_service = '$server' ");
            ...
            if($soluong == 0){
                ...
                if($submit_order){
                    ...
                    if($update_user){
                        send_mess("Đã có 1 đơn hàng like post sale vui lòng duyệt");
                        echo requestJson(200,true ,"Đã thêm vào danh sách giao dịch vui lòng chờ xử lý");
                    }else{
