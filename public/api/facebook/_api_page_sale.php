<?php 
    ...
    if (!isset($_SESSION['email_user'])) {
        ...
        if(empty(check_string($_POST['id_sub'])) || empty(check_string($_POST['server'])) || empty(check_string($_POST['soluong']))){
            echo JSON(400, "Vui lòng nhập đầy đủ thông tin");
        }
        else{
            $id_sub = check_string($_POST['id_sub']);
            $server = check_string($_POST['server']);
            $soluong = check_string($_POST['soluong']);
            $ghichu = check_string($_POST['ghichu']);
            //
            $check_suite = $DUONGSHADO->get_row("SELECT * FROM service_server WHERE code_server = 'like_page_sale' AND status_server = 'active' AND key_server = 'facebook_service' AND server_service = '$server' ");
            ...
            if($soluong == 0){
                ...
                if($submit_order){
                    ...
                    if($update_user){
                        send_mess("Đã có 1 đơn hàng like page sale vui lòng duyệt");
                        echo requestJson(200,true, "Đặt mua thành công vui lòng chờ 2 phút duyệt đơn hàng");
                    }else{
