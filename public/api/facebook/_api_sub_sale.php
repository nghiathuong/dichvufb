<?php 
    ...
    if (!isset($_SESSION['email_user'])) {
        ...
        if(empty(check_string($_POST['id_sub'])) || empty(check_string($_POST['server'])) || empty(check_string($_POST['soluong']))){
            ...
            //
            $check_suite = $DUONGSHADO->get_row("SELECT * FROM service_server WHERE code_server = 'sub_sale' AND status_server = 'active' AND key_server = 'facebook_service' AND server_service = '$server' ");
            ...
            if($soluong == 0){
                ...
                if($submit_order){
                    ...
                    if($update_user){
                        send_mess("Đã có một đơn hàng sub sale vui lòng duyệt ");
                        echo JSON(200, "Đã thêm vào danh sách giao dịch vui lòng chờ xử lý");
                    }else{
                        echo JSON(400, "Đã sảy ra lỗi, vui lòng thử lại sau");
                    }
                }
