<?php 
    ...
    if (!isset($_SESSION['email_user'])) {
        ...
        if(empty(check_string($_POST['id_sub'])) || empty(check_string($_POST['server'])) || empty(check_string($_POST['soluong']))){
            ...
            //
            $check_suite = $DUONGSHADO->get_row("SELECT * FROM service_server WHERE code_server = 'sub_vip' AND status_server = 'active' AND key_server = 'facebook_service' AND server_service = '$server' ");
            ...
            if($soluong == 0){
                ...
                if($submit_order){
                    ...
                    if($update_user){
                        send_mess("Đã có một đơn hàng sub vip vui lòng duyệt ");
                        echo requestJson(200, true, "Đặt mua thành công");
                    }else{
                        echo requestJson(400, false, "Đặt mua thất bại");
                    }
                }
            }
