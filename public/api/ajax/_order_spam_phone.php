<?php 
    ...
    if (!isset($_SESSION['email_user'])) {
        ...
        if(empty(check_string($_POST['phone_number'])) || empty(check_string($_POST['server'])) || empty(check_string($_POST['cmt_post'])) || empty(check_string($_POST['soluong']))){
            echo JSON(400, "Vui lòng nhập đầy đủ thông tin");
        }
        else{
            $link_post = check_string($_POST['phone_number']);
            $server = check_string($_POST['server']);
            $cmt_post = check_string($_POST['cmt_post']);
            $soluong = check_string($_POST['soluong']);
            $check_suite = $DUONGSHADO->get_row("SELECT * FROM service_server WHERE code_server = 'spam_cmt_phone' AND status_server = 'active' AND key_server = 'orther_service' AND server_service = '$server' ");
            ...
            if($soluong == 0){
                ...
                if($submit_order){
                    ...
                    if($update_user){
                        send_mess("Đã có một đơn hàng cmt phone vui lòng duyệt ");
